...
* identifier 0..0 SU Identifier "External Identifier" "Id for specimen."
* accessionIdentifier 0..1 SU Identifier "Identifier assigned by the lab" "The identifier assigned by the lab when accessioning specimen(s). This is not necessarily the same as the specimen identifier, depending on local lab procedures."
* status 0..1 ?! SU code "available | unavailable | unsatisfactory | entered-in-error" "The availability of the specimen."
...
* type 0..1 SU CodeableConcept "Kind of material that forms the specimen" "The kind of material that forms the specimen."
...
* receivedTime 0..1 SU dateTime "The time when specimen was received for processing" "Time when specimen was received for processing or testing."
* parent 0..0 Reference(http://hl7.org/fhir/StructureDefinition/Specimen) "Specimen from which this specimen originated" "Reference to the parent (source) specimen which is used when the specimen was either derived from or a component of another specimen."
...
* request 0..0 Reference(http://hl7.org/fhir/StructureDefinition/ServiceRequest) "Why the specimen was collected" "Details concerning a service request that required a specimen to be collected."
* request ^comment = "The request may be explicit or implied such with a ServiceRequest that requires a blood draw."
* collection 0..1 BackboneElement "Collection details" "Details concerning the specimen collection."
* collection.collector 0..1 SU Reference(http://hl7.org/fhir/StructureDefinition/Practitioner or http://hl7.org/fhir/StructureDefinition/PractitionerRole) "Who collected the specimen" "Person who collected the specimen."
* collection.collected[x] 0..1 SU dateTime or Period "Collection time" "Time when specimen was collected from subject - the physiologically relevant time."
* collection.duration 0..1 SU Duration "How long it took to collect specimen" "The span of time over which the collection of a specimen occurred."
* collection.quantity 0..1 http://hl7.org/fhir/StructureDefinition/SimpleQuantity "The quantity of specimen collected" "The quantity of specimen collected; for instance the volume of a blood sample, or the physical measurement of an anatomic pathology sample."
* collection.method 0..1 CodeableConcept "Technique used to perform collection" "A coded value specifying the technique that is used to perform the procedure."
...
* collection.bodySite 0..1 CodeableConcept "Anatomical collection site" "Anatomical location from which the specimen was collected (if subject is a patient). This is the target site.  This element is not used for environmental specimens."
...
* collection.fastingStatus[x] 0..1 SU CodeableConcept or Duration "Whether or how long patient abstained from food and/or drink" "Abstinence or reduction from some or all food, drink, or both, for a period of time prior to sample collection."
...
* processing 0..0 BackboneElement "Processing and processing step details" "Details concerning processing and processing steps for the specimen."
* processing.description 0..1 string "Textual description of procedure" "Textual description of procedure."
* processing.procedure 0..1 CodeableConcept "Indicates the treatment step  applied to the specimen" "A coded value specifying the procedure used to process the specimen."
...
* processing.additive 0..0 Reference(http://hl7.org/fhir/StructureDefinition/Substance) "Material used in the processing step" "Material used in the processing step."
* processing.time[x] 0..1 dateTime or Period "Date and time of specimen processing" "A record of the time or period when the specimen processing occurred.  For example the time of sample fixation or the period of time the sample was in formalin."
* container 0..0 BackboneElement "Direct container of specimen (tube/slide, etc.)" "The container holding the specimen.  The recursive nature of containers; i.e. blood in tube in tray in rack is not addressed here."
* container.identifier 0..0 SU Identifier "Id for the container" "Id for container. There may be multiple; a manufacturer's bar code, lab assigned identifier, etc. The container ID may differ from the specimen id in some circumstances."
* container.description 0..1 string "Textual description of the container" "Textual description of the container."
* container.type 0..1 CodeableConcept "Kind of container directly associated with specimen" "The type of container associated with the specimen (e.g. slide, aliquot, etc.)."
...
* container.capacity 0..1 http://hl7.org/fhir/StructureDefinition/SimpleQuantity "Container volume or size" "The capacity (volume or other measure) the container may contain."
* container.specimenQuantity 0..1 http://hl7.org/fhir/StructureDefinition/SimpleQuantity "Quantity of specimen within container" "The quantity of specimen in the container; may be volume, dimensions, or other appropriate measurements, depending on the specimen type."
* container.additive[x] 0..1 CodeableConcept or Reference(http://hl7.org/fhir/StructureDefinition/Substance) "Additive associated with container" "Introduced substance to preserve, maintain or enhance the specimen. Examples: Formalin, Citrate, EDTA."
...
* condition 0..0 SU CodeableConcept "State of the specimen" "A mode or state of being that describes the nature of the specimen."
...
* note 0..0 Annotation "Comments" "To communicate any details or issues about the specimen or during the specimen collection. (for example: broken vial, sent with patient, frozen)."