...
* identifier 1..0 MS SU Identifier "An identifier for this patient" "An identifier for this patient. At least one identifier, the IHS Number is mandatory."
* identifier ^requirements = "Patients are almost always assigned specific numerical identifiers."
* identifier.use 0..1 BackboneElement "The purpose of this identifier. For IHS Number must equal \"official\"."
* identifier.type 1..0 BackboneElement "A coded type for the identifier that can be used to determine which identifier to use for a specific purpose. For IHS Number must equal \"IHS\"."
* identifier.system 1..0 BackboneElement "For IHS Number, \"system\" must equal to \"https://fhir.kemkes.go.id/id/ihs-number\"\r\nFor NIK, \"system\" must equal to \"https://fhir.kemkes.go.id/id/nik\""
* identifier.value 1..0 BackboneElement
...
* gender 1..1 SU code "female" "Administrative Gender - the gender that the patient is considered to have for administration and record keeping purposes."
...
* birthDate.extension[birthTime].value[x] 0..0 BackboneElement "Datetime of birth" "The time of day that the patient was born. This includes the date to ensure that the timezone information can be communicated effectively."
...
* address 0..0 SU https://fhir.kemkes.go.id/r4/StructureDefinition/Address "An address for the individual" "An address for the individual."
...
* address.extension[geolocation].extension 1..0 BackboneElement
...
* multipleBirth[x] 0..1 boolean or integer "Whether patient is part of a multiple birth" "Indicates whether the patient is part of a multiple (boolean) or indicates the actual birth order (integer)."
...
* photo 0..0 Attachment "Image of the patient" "Image of the patient."
...
* contact 0..0 BackboneElement "A contact party (e.g. guardian, partner, friend) for the patient" "A contact party (e.g. guardian, partner, friend) for the patient."
* contact obeys pat-1
...
* contact.relationship 0..0 CodeableConcept "The kind of relationship" "The nature of the relationship between the patient and the contact person."
...
* contact.telecom 0..0 ContactPoint "A contact detail for the person" "A contact detail for the person, e.g. a telephone number or an email address."
...
* communication 0..0 BackboneElement "A language which may be used to communicate with the patient about his or her health" "A language which may be used to communicate with the patient about his or her health."
...
* generalPractitioner 0..0 Reference(https://fhir.kemkes.go.id/r4/StructureDefinition/Organization or https://fhir.kemkes.go.id/r4/StructureDefinition/Practitioner or https://fhir.kemkes.go.id/r4/StructureDefinition/PractitionerRole) "Patient's nominated primary care provider" "Patient's nominated care provider."
...
* link 0..0 ?! SU BackboneElement "Link to another patient resource that concerns the same actual person" "Link to another patient resource that concerns the same actual patient."
